...
This runs the pre-provision hook to resolve UPNs, deploys all Azure resources with role assignments embedded in the Bicep deployment, then runs the post-provision hook to generate per-attendee onboarding files and seed the Azure AI Search indexes. Re-run this command any time you change `AZURE_ATTENDEE_LIST`, `AZURE_ATTENDEE_COUNT`, or the project prefix.
...
* The shared connection values for `.env`: `AZURE_SUBSCRIPTION_ID`, `AZURE_RESOURCE_GROUP`, `FOUNDRY_RESOURCE_NAME`, `FOUNDRY_PROJECT_ENDPOINT`, `AZURE_OPENAI_ENDPOINT`, and `AZURE_SEARCH_SERVICE_NAME`.

> [!TIP]
> The per-attendee onboarding file generated by `scripts/generate-attendee-onboarding.py` (written to `.azure/<upn_local>.md`) includes all of these values pre-populated, including the computed `FOUNDRY_PROJECT_ENDPOINT` and `AZURE_OPENAI_ENDPOINT`.
...
markdown file (`.azure/<upn_local>.md`) and a provisioning summary CSV.
