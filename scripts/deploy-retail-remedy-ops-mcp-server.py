"""Build and deploy the Retail Remedy Operations MCP server image to its Azure Container App.
...
_MCP_SERVER_DIR = _REPO_ROOT / 'shared' / 'mcp-servers' / 'retail-remedy-ops'
...
def main() -> int:  # pylint: disable=too-many-return-statements
    ...
    if not _is_truthy(deploy_enabled):
        print(
            ...
            '(AZURE_CONTAINER_APPS_DEPLOY=false). Skipping MCP server image deploy.'
        )
        return 0
    ...
    if not container_app_name:
        return _fail(
            'The MCP server Container App is not provisioned. '
    ...
    print(f'{TICK} MCP server deployed.')
    if mcp_server_url:
