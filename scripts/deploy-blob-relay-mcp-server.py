"""Build and deploy the Blob Relay MCP server image to its Azure Container App.
...
_MCP_SERVER_DIR = _REPO_ROOT / 'shared' / 'mcp-servers' / 'blob-relay'
...
def main() -> int:  # pylint: disable=too-many-return-statements
    ...
    if not _is_truthy(deploy_enabled):
        print(
            ...
            '(AZURE_CONTAINER_APPS_DEPLOY=false). Skipping Blob Relay MCP server image deploy.'
        )
        return 0
    ...
    container_app_name = env.get('AZURE_BLOB_RELAY_MCP_SERVER_CONTAINER_APP_NAME', '')
    mcp_server_url = env.get('BLOB_RELAY_MCP_SERVER_URL', '')

    if not container_app_name:
        return _fail(
            'The Blob Relay MCP server Container App is not provisioned. '
    ...
    image = f'{login_server}/blob-relay:{tag}'
    ...
    print(f'{TICK} Blob Relay MCP server deployed.')
    if mcp_server_url:
