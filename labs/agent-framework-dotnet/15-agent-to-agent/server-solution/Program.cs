...
// ── Register the Compensation Specialist as a keyed singleton AIAgent ────────
...
// ── Register the A2A server for this agent ────────────────────────────────────
builder.AddA2AServer(AgentName);

var app = builder.Build();

// ── Map the A2A endpoint and publish the well-known agent card ───────────────
...
Console.ForegroundColor = ConsoleColor.DarkGray;
Console.WriteLine("[Loop] Serving compensation-specialist at /a2a/compensation-specialist");
Console.WriteLine("[Loop] Agent card published at /.well-known/agent-card.json");
Console.ResetColor();
