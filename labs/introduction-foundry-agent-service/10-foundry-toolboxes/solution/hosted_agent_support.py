"""Shared helpers for deploying and operating the Module 10 hosted agent.

These helpers are used by both the deployment script (``deploy_hosted_agent_code.py``) and
the invocation script (``invoke_hosted_agent.py``) so the lab keeps a single source of truth
for polling, version selection, and agent-identity RBAC. They mirror the Module 09 helpers so
each lab module stays independently runnable.
"""
...
def get_latest_active_agent_version(client: AIProjectClient, agent_name: str) -> AgentVersionDetails:
    ...
    raise RuntimeError(
        ...
        'Deploy a version first with deploy_hosted_agent_code.py.'
    )
